...
!!! LET OP — DIT SCRIPT IS VEROUDERD, NIET ZOMAAR DRAAIEN (nagekeken 02-09-2026) !!!

Twee dingen kloppen niet meer:

1. BRONMAP. Dit script zoekt markdown in blog/*.md, maar de bronbestanden staan
   sinds een verhuis in _bron/blog/*.md. Er staat dus GEEN enkele .md meer in
   blog/. Het script doet vandaag letterlijk niets: het print "Geen .md files
   gevonden" en stopt voor het index.html aanraakt.

2. SJABLOON. Belangrijker: de HTML in blog/ is sindsdien met de hand veel verder
   gebracht dan wat HEAD/ARTICLE_WRAPPER/FOOTER hieronder genereren. Wat er in de
   live HTML zit en NIET in dit sjabloon:
     - <link rel="canonical"> en og:image / twitter:card
     - JSON-LD (Article + BreadcrumbList) voor Google
     - de Meta Pixel (GDPR-gated) + consent.js + intake-widget.js
     - de huidige nav met logo-SVG, mobiel menu (Alpine) en kruimelpad
     - de huidige footer (sectorpagina's, security-link)
     - .table-wrap om brede tabellen, en de .post-meta regel met datum/leestijd
     - de merkkleuren van na de rebrand (ink/brand/mint i.p.v. ink/accent)

   Wie enkel de glob hierboven "fixt" en dit draait, gooit dat allemaal weg en
   zet tien artikels terug op een sjabloon van voor de rebrand. Dat is een
   stille SEO- en trackingregressie.

WERKWIJZE ZOLANG DIT NIET HERSCHREVEN IS:
   Pas artikels op TWEE plaatsen aan en houd ze gelijk:
     - de bron:  _bron/blog/<slug>.md
     - de output: blog/<slug>.html  (enkel het <article>-blok + de titels in
       <title>, og:title, JSON-LD headline en het kruimelpad)

Wil je dit script weer bruikbaar maken, dan moet het sjabloon eerst gelijkgezet
worden met de huidige blog/*.html en moet BLOG_DIR/de bronmap gesplitst worden.
...
# Veiligheidsslot. Het sjabloon in dit bestand loopt achter op de live HTML
# (canonical, JSON-LD, Meta Pixel, huidige nav/footer, .table-wrap, post-meta).
# Draaien overschrijft blog/*.html met een oudere, armere versie.
# Zet dit pas op True nadat HEAD/ARTICLE_WRAPPER/FOOTER gelijkgezet zijn met
# de huidige blog/*.html. Zie de uitleg bovenaan dit bestand.
SJABLOON_IS_BIJGEWERKT = False


def main():
    if not SJABLOON_IS_BIJGEWERKT:
        print(
            "AFGEBROKEN: build_blog.py is verouderd en zou de blog-HTML "
            "terugzetten op een sjabloon van voor de rebrand.\n"
            "Bewerk voorlopig _bron/blog/<slug>.md én blog/<slug>.html met de hand.\n"
            "Zie de toelichting bovenaan build_blog.py."
        )
        return
